...
mount_hole_min_edge    = 4;   // Minimum edge clearance for rail-to-plate mounting holes (mm)
mount_hole_min_spacing = 8;   // Minimum center-to-center spacing between the two holes (mm)
...
// Get effective fan size (respects override or uses database value)
function get_effective_fan_size(ctrl) =
    fan_size_override == 0 ? get_fan_type(ctrl) :  // AutoSelect - use database
    fan_size_override;  // Use override value (40 or 50)

// Get rail height using default (database) fan size - used for stable hole positioning
// Hole positions must NOT change when fan_size_override is active
function get_default_rail_height(ctrl) =
    (get_fan_type(ctrl) + 4) - get_heatsink_height(ctrl);

// Determine whether two holes can fit in a given actual rail height.
// When holes would be closer than mount_hole_min_spacing, fall back to one centered hole.
function use_two_mount_holes(ctrl, actual_rail_h) =
    let(
        default_rail_h = get_default_rail_height(ctrl),
        h1 = default_rail_h * 0.25,
        h2 = min(default_rail_h * 0.75, actual_rail_h - mount_hole_min_edge)
    )
    (h2 - h1) >= mount_hole_min_spacing;

function mount_hole_1(ctrl) = get_default_rail_height(ctrl) * 0.25;
function mount_hole_2(ctrl, actual_rail_h) =
    min(get_default_rail_height(ctrl) * 0.75, actual_rail_h - mount_hole_min_edge);
function mount_hole_center(actual_rail_h) = actual_rail_h / 2;
...
if (ctrl == undef) {
    ...
    eff_rail_h = (effective_fan_size + 4) - get_heatsink_height(ctrl);
    single_hole_mode = !use_two_mount_holes(ctrl, eff_rail_h);
    ...
    echo(str("Rail height: ", eff_rail_h, "mm | Mount holes: ", single_hole_mode ? "1 (centered - rail too short for 2)" : "2"));
    echo(str("Fans: ", effective_fan_count, "×", effective_fan_size, "mm (", fan_mode, ")", recalc_notice));
...
module front_fan_mount(ctrl) {
    fan_count = get_effective_fan_count(ctrl);  // Use effective count (recalculates if override active)
    fan_size = get_effective_fan_size(ctrl);
    ...
    difference() {
        ...
        // Rail mounting holes - positions fixed to default fan size so override doesn't shift them
        rail_hole_dia = m4_hole_dia;
        edge_offset = 8;  // Distance from plate side edges

        actual_rail_h = (get_effective_fan_size(ctrl) + 4) - get_heatsink_height(ctrl);
        two_holes     = use_two_mount_holes(ctrl, actual_rail_h);
        h1            = mount_hole_1(ctrl);
        h2            = mount_hole_2(ctrl, actual_rail_h);
        hc            = mount_hole_center(actual_rail_h);
        ...
        if (two_holes) {
            ...
            // Rail too short for two spaced holes — use single centered hole
            translate([edge_offset,             hc, -0.5]) cylinder(d=rail_hole_dia, h=plate_thickness+1);
            translate([plate_width-edge_offset, hc, -0.5]) cylinder(d=rail_hole_dia, h=plate_thickness+1);
        }
    ...
    echo(str("Front fan mount: ", plate_width, "x", plate_length, "mm plate with ", fan_count, "x", fan_size, "mm fans"));
}
...
module left_rail(ctrl) {
    ...
    union() {
        ...
        // Holes to connect to front/rear plates - fixed positions, falls back to single if rail too short
        two_holes = use_two_mount_holes(ctrl, rail_height);
        h1        = mount_hole_1(ctrl);
        h2        = mount_hole_2(ctrl, rail_height);
        hc        = mount_hole_center(rail_height);

        if (two_holes) {
            // Front face
            translate([rail_width/2, 0, h1]) rotate([90, 0, 0]) cylinder(d=m4_hole_dia, h=12, center=true);
            translate([rail_width/2, 0, h2]) rotate([90, 0, 0]) cylinder(d=m4_hole_dia, h=12, center=true);
            // Rear face
            translate([rail_width/2, rail_length, h1]) rotate([90, 0, 0]) cylinder(d=m4_hole_dia, h=12, center=true);
            translate([rail_width/2, rail_length, h2]) rotate([90, 0, 0]) cylinder(d=m4_hole_dia, h=12, center=true);
        } else {
            // Rail too short for two spaced holes — use single centered hole
            translate([rail_width/2, 0,           hc]) rotate([90, 0, 0]) cylinder(d=m4_hole_dia, h=12, center=true);
            translate([rail_width/2, rail_length, hc]) rotate([90, 0, 0]) cylinder(d=m4_hole_dia, h=12, center=true);
        }
...
module rear_grill(ctrl) {
    ...
    plate_width = get_total_width(ctrl) + plate_width_adjust;     // Total width + adjustment
    plate_length = get_effective_fan_size(ctrl) + 4;          // Fan height + 4mm spacing
    plate_thickness = plate_depth;                   // 6mm thick
